...
class ShogiGame(AbstractGame):
    ...
    def _init_board(self):
        ...
        # First rank (top)
        self.board[0] = [
            ShogiPiece('L', 'b'), ShogiPiece('N', 'b'), ShogiPiece('S', 'b'),
            ShogiPiece('G', 'b'), ShogiPiece('K', 'b'), ShogiPiece('G', 'b'),
            ShogiPiece('S', 'b'), ShogiPiece('N', 'b'), ShogiPiece('L', 'b')
        ]
        # Second rank
        self.board[1] = [
            None, None, None, ShogiPiece('B', 'b'), None, ShogiPiece('R', 'b'), None, None, None
        ]
        # Third rank
        ...
        # Eighth rank
        self.board[7] = [
            None, None, None, ShogiPiece('B', 'w'), None, ShogiPiece('R', 'w'), None, None, None
        ]
        # Ninth rank (bottom)
        self.board[8] = [
            ShogiPiece('L', 'w'), ShogiPiece('N', 'w'), ShogiPiece('S', 'w'),
            ShogiPiece('G', 'w'), ShogiPiece('K', 'w'), ShogiPiece('G', 'w'),
            ShogiPiece('S', 'w'), ShogiPiece('N', 'w'), ShogiPiece('L', 'w')
        ]
            ...
            for move in self.history_state:
                ...
                if move_data.get('is_drop'):
                    self.hands[move_data['color']][move_data['piece_type']] = (
                        self.hands[move_data['color']].get(move_data['piece_type'], 0) - 1
                    )
    ...
    def validate_move(self, move_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            return self._validate_piece_move(piece.piece_type, piece.promoted, (from_row, from_col), (to_row, to_col))
    ...
    def _validate_drop(self, move_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if piece_type not in self.hands[self.current_player] or \
               self.hands[self.current_player][piece_type] <= 0:
                return False
            ...
            if piece_type == 'P':  # Pawn
                ...
                for row in range(self.BOARD_SIZE):
                    if self.board[row][to_col] and self.board[row][to_col].piece_type == 'P' and \
                       self.board[row][to_col].color == self.current_player:
                        return False
    ...
    def _validate_piece_move(self, piece_type: str, promoted: bool, from_pos: Tuple[int, int], to_pos: Tuple[int, int]) -> bool:
        """Validate move based on piece type"""
        ...
        if piece_type == 'P':  # Pawn
            ...
            return (to_row == from_row + (2 * direction) and 
    ...
    def _validate_promoted_pawn_move(self, from_pos: Tuple[int, int], to_pos: Tuple[int, int]) -> bool:
    ...
    def _validate_promoted_lance_move(self, from_pos: Tuple[int, int], to_pos: Tuple[int, int]) -> bool:
    ...
    def _validate_promoted_knight_move(self, from_pos: Tuple[int, int], to_pos: Tuple[int, int]) -> bool:
    ...
    def _validate_promoted_silver_move(self, from_pos: Tuple[int, int], to_pos: Tuple[int, int]) -> bool:
    ...
    def _validate_promoted_bishop_move(self, from_pos: Tuple[int, int], to_pos: Tuple[int, int]) -> bool:
        """Validate promoted bishop move"""
        from_row, from_col = from_pos
        to_row, to_col = to_pos
        
        return (abs(to_row - from_row) == abs(to_col - from_col)) or \
               (abs(to_row - from_row) <= 1 and 
                abs(to_col - from_col) <= 1)

    def _validate_promoted_rook_move(self, from_pos: Tuple[int, int], to_pos: Tuple[int, int]) -> bool:
    ...
    def _can_attack(self, piece: ShogiPiece, from_pos: Tuple[int, int], to_pos: Tuple[int, int]) -> bool:
        """Check if a piece can attack a position"""
        temp_piece = self.board[to_pos[0]][to_pos[1]]
        self.board[to_pos[0]][to_pos[1]] = None
        
        can_attack = self._validate_piece_move(piece.piece_type, piece.promoted, from_pos, to_pos)
    ...
    def make_move(self, move_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if piece.piece_type in ['P', 'L', 'N', 'S'] and \
           (piece.color == 'w' and to_row <= 2 or 
            piece.color == 'b' and to_row >= 6):
            piece.promoted = True
