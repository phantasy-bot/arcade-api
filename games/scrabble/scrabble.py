...
class Tile:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"letter": self.letter, "points": self.points, "is_blank": self.is_blank}
...
class ScrabbleGame(AbstractGame):
    ...
    def _calculate_word_score(
        self, word: List[Tuple[int, int, str]], is_first_move: bool = False
    ) -> int:
    ...
    def validate_move(self, move_data: Dict[str, Any]) -> bool:
        ...
        if action == "place_word":
            if (
                "word" not in move_data
                or "start" not in move_data
                or "direction" not in move_data
            ):
                return False
    ...
    def _is_valid_position(
        self, start: Tuple[int, int], direction: str, word: str
    ) -> bool:
    ...
    def make_move(self, move_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if action == "place_word":
            ...
            # Draw new tiles
            self._draw_tiles(
                self.current_player, 7 - len(self.rack[self.current_player])
            )

            # Check if game is over
            if not self.bag and all(
                len(self.rack[p]) == 0 for p in range(self.num_players)
            ):
                self.phase = "end"
    ...
    def get_game_state(self) -> Dict[str, Any]:
        ...
        return {
            "board": [
                [tile.letter if tile else None for tile in row] for row in self.board
            ],
            "current_player": self.current_player,
            "phase": self.phase,
            "rack": {
                str(k): [tile.to_dict() for tile in v] for k, v in self.rack.items()
            },
            "score": self.score,
            "turns": self.turns,
