...
class GoGame(AbstractGame):
    ...
    def _capture_groups(self, pos: Tuple[int, int]) -> None:
        ...
        for neighbor in self._get_neighbors(pos):
            if self.board[neighbor] != 0 and self.board[neighbor] != self.current_player.value:
                group = self._find_group(neighbor)
                if group and group.is_captured():
                    self.captured_stones[GoStone(self.board[neighbor])] += len(group.stones)
    ...
    def make_move(self, move_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'pass' in move_data and move_data['pass']:
            ...
            self.current_player = GoStone.WHITE if self.current_player == GoStone.BLACK else GoStone.BLACK
    ...
    def get_winner(self) -> Optional[str]:
        ...
        scores = {
            GoStone.BLACK: territory[GoStone.BLACK] + self.captured_stones[GoStone.BLACK],
            GoStone.WHITE: territory[GoStone.WHITE] + self.captured_stones[GoStone.WHITE] + 7.5  # Komi
        }
