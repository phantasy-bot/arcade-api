...
class BattleshipGame(AbstractGame):
    ...
    def _init_game(self):
        ...
        self.current_player = 0
        self.phase = "setup"
        self.boards = defaultdict(
            lambda: np.zeros((self.board_size, self.board_size), dtype=int)
        )
        self.shots = defaultdict(
            lambda: np.zeros((self.board_size, self.board_size), dtype=int)
        )
        self.ships = defaultdict(dict)
        self.sunk_ships = defaultdict(set)
        ...
        for ship_type, (pos, ship_length, horizontal) in self.ships[player].items():
            if horizontal:
                if pos[0] <= x < pos[0] + ship_length and pos[1] == y:
                    ...
                    if all(
                        self.shots[player][pos[0] + i][pos[1]] != 0
                ...
                if pos[0] == x and pos[1] <= y < pos[1] + ship_length:
                    # Check if all positions of this ship have been hit
                    if all(
                        self.shots[player][pos[0]][pos[1] + i] != 0
                        for i in range(ship_length)
                    ):
                        return ship_type
    ...
    def get_game_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "shots": self.shots,
            "sunk_ships": {
                str(k): [s.name for s in v] for k, v in self.sunk_ships.items()
            },
